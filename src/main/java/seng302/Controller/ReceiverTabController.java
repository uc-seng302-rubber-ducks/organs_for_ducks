...
public class ReceiverTabController {
  ...
  public void registerOrgan() {
    if (organsComboBox.getSelectionModel().getSelectedItem() != null) {
      ...
      if (!currentlyReceivingListView.getItems().contains(toRegister)) {
        ...
        Log.info("Successfully registered organ for receiver");
      } else {
        Log.warning("Unable to register organ for receiver as organ "+ toRegister.organName +" has already been registered");
      }

      parent.refreshCurrentlyDonating();
      currentlyReceivingListView.refresh();
    } else {
      Log.warning("Unable to register organ for receiver as no organ selected");
    }
  }
  ...
  public void reRegisterOrgan() {
    ...
    if (toReRegister != null) {
      ...
      AppController.getInstance().getClinicianController().refreshTables();
      Log.info("Successfully re-registered organ for receiver");
      ...
      Log.warning("Unable to re-register organ for receiver as no organ selected");
    }
  ...
  private void deregisterOrganReason() {
    ...
    if (toDeRegister != null) {
      ...
      try {
        ...
        stage.show();
        Log.info("Successfully launched deregister organ reason window");
      } catch (IOException e) {
        Log.severe("unable to launch deregister organ reason window", e);
        e.printStackTrace();
      }
    }
  }
  ...
  private void launchReceiverOrganDateView(Organs organs) {
    ...
    try {
      ...
      stage.show();
      Log.info("Successfully launched register and deregister time table window");
    } catch (IOException e) {
      Log.severe("unable to launch register and deregister time table window", e);
      e.printStackTrace();
    }
  }
  ...
  public void deRegisterOrgan(Organs toDeRegister) {
    if (toDeRegister != null) {
      ...
      if (organDeregisterationReason == OrganDeregisterReason.TRANSPLANT_RECEIVED) {
        ...
        Log.info("Successfully de-registered organ for receiver");
      ...
      Log.warning("Unable to de-register organ for receiver as no organ selected");
    }
  }
